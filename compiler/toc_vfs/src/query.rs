...
pub trait FileSystem: salsa::Database + HasVFS + PathInterner {
    /// `PathBuf` that serves as the root of all compilation.
    #[salsa::input]
    fn root_file_path(&self) -> std::path::PathBuf;
    ...
    fn file_source(&self, file: FileId) -> Arc<(String, Option<fs_impl::LoadError>)>;
}

#[salsa::query_group(PathInternStorage)]
pub trait PathInterner {
    #[salsa::interned]
    fn intern_path(&self, path: std::path::PathBuf) -> InternedFileId;
}
...
fn resolve_path(db: &dyn FileSystem, relative_to: FileId, relative_path: &str) -> FileId {
    ...
    let expanded_path = expand_path(relative_path);
    ...
    let full_path = if expanded_path.is_absolute() {
        ...
        let mut parent_path: std::path::PathBuf =
            db.lookup_intern_path(InternedFileId(relative_to));
        parent_path.pop();
...
fn file_source(db: &dyn FileSystem, file: FileId) -> Arc<(String, Option<fs_impl::LoadError>)> {
    db.salsa_runtime()
...
fn expand_path(path: impl AsRef<std::path::Path>) -> std::path::PathBuf {
    use std::path::{Component, Path, PathBuf};

    fn inner(path: &Path) -> PathBuf {
        // Check if the given path has a percent prefix
        let _prefix_name = if_chain::if_chain! {
...
pub trait HasVFS {
    /// Gets access to the underlying VFS
    fn get_vfs(&self) -> &dyn fs_impl::FsBackend;
}
