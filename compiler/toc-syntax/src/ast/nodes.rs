//! Generated by `xtask codegen grammar`, do not edit by hand
...
impl RangeType {
    pub fn packed_token(&self) -> Option<SyntaxToken> {
        helper::token(&self.0, SyntaxKind::KwPacked)
    }
    pub fn range_token(&self) -> Option<SyntaxToken> { helper::token(&self.0, SyntaxKind::Range) }
    pub fn size_spec(&self) -> Option<SizeSpec> { helper::node(&self.0) }
}
...
impl AstNode for ParamDecl {
    ...
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        match syntax.kind() {
            ...
            _ if SubprogType::can_cast(syntax.kind()) => {
                Some(Self::SubprogType(AstNode::cast(syntax)?))
            }
            _ => None,
        }
    }
