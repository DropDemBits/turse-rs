...
// FIXME(salsa-202): consider eliding generated return ref if it's clear that it can be elided (or just add the allow)

pub use expansion::{BuiltinPrefix, PrefixExpansions};
pub use paths::RawPath;

#[salsa::jar(db = Db)]
pub struct Jar(
    paths::RawPath,
    paths::RawPath_expand_path,
...
mod paths {
    ...
    use crate::{BuiltinPrefix, Db, PrefixExpansions};
    ...
    #[salsa::tracked]
    impl RawPath {
        /// Expands any prefixes in the raw path
        #[allow(clippy::needless_lifetimes)] // shusheth, this is autogenerated and we don't have access to the lifetime
        #[salsa::tracked(return_ref)]
        pub fn expand_path(self, db: &dyn Db) -> Utf8PathBuf {
            fn try_it(db: &dyn Db, path: RawPath) -> Option<Utf8PathBuf> {
                let raw_path = path.raw_path(db);

                let prefix_name = {
                    // Try parsing out a percent prefix
                    let Some(Utf8Component::Normal(comp)) = raw_path.components().next() else { return None;};
                    comp.strip_prefix('%')?
                };
                ...
                Some(base_path.join(raw_path.strip_prefix(prefix_path.percent_prefix()).unwrap()))
            }

            try_it(db, self).unwrap_or_else(|| self.raw_path(db).to_owned())
        }
    }
}
